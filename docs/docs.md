...
Posts are constructed in [markdown](http://daringfireball.net/projects/markdown/)/[pug](https://pugjs.org/api/getting-started.html)/[whatever-you-want](#Templates), prefixed by front matter via [YAML](https://github.com/mojombo/jekyll/wiki/YAML-Front-Matter) or [JSON](https://github.com/jsantell/node-json-front-matter). All attributes are stored in the posts object. An example of a blog post formatted with JSON Front Matter is below:
